...
public class JSQLParserFluentModelTests {
    ...
    public void testParseAndBuild() throws JSQLParserException {
        ...
        ExpressionList list = select.getWhere(AndExpression.class).getRightExpression(InExpression.class).getRightItemsList(ExpressionList.class);
    ...
    public void testParseAndBuildForXOR() throws JSQLParserException {
        ...
        PlainSelect select = new PlainSelect().addSelectItems(new AllColumns()).withFromItem(t1).addJoins(new Join().withRightItem(t2).withOnExpression(new EqualsTo(new Column(asList("t1", "ref")), new Column(asList("t2", "id"))))).withWhere(where);
        ExpressionList list = select.getWhere(XorExpression.class).getLeftExpression(AndExpression.class).getRightExpression(InExpression.class).getRightItemsList(ExpressionList.class);
        List<Expression> elist = list.getExpressions();
        list.setExpressions(elist);
        assertDeparse(select, statement);
        assertEqualsObjectTree(parsed, select);
