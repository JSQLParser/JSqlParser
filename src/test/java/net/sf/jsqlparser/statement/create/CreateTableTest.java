...
public class CreateTableTest {
    ...
    public void testTimestampWithTimezone() throws JSQLParserException {
        assertSqlCanBeParsedAndDeparsed(
                "CREATE TABLE country_region (" + "regionid BIGINT NOT NULL CONSTRAINT pk_auth_region PRIMARY KEY, "
                        ...
                        for (Iterator iter = createTable.getColumnDefinitions().iterator(); iter.hasNext();) {
                            ...
                            if (!unique) {
                                if (columnDefinition.getColumnSpecs() != null) {
                                    for (Iterator iterator = columnDefinition.getColumnSpecs().iterator(); iterator
                                            .hasNext();) {
    ...
    public void testSettingCharacterSetIssue829() throws JSQLParserException {
        ...
        ColumnDefinition colName = stmt.getColumnDefinitions().stream()
                .filter(col -> col.getColumnName().equals("name")).findFirst().orElse(null);
