...
public class SelectTest {
  ...
  void checkMultipartIdentifier(Select select, String columnName, String fullColumnName) {
    final Expression expr =
        ((SelectExpressionItem) ((PlainSelect) select.getSelectBody()).getSelectItems().get(0))
            .getExpression();
  ...
  public void testAllColumnsFromTable() throws Exception {
    ...
    assertStatementCanBeDeparsedAs(select, statement);
    assertTrue(
        ((PlainSelect) select.getSelectBody()).getSelectItems().get(0) instanceof AllTableColumns);
    ...
    assertDeparse(
        new Select()
            .withSelectBody(
                new PlainSelect()
                    .addSelectItems(new AllTableColumns().withTable(t))
                    .withFromItem(t)),
        statement);
  }
  ...
  public void testLimit() throws JSQLParserException {
    ...
    assertEquals(
        "?",
        ((PlainSelect) select.getSelectBody()).getOffset().getOffsetJdbcParameter().toString());
    assertStatementCanBeDeparsedAs(select, statement);
  ...
  public void testLimitSqlServerJdbcParameters() throws JSQLParserException {
    ...
    assertEquals(
        ...
        ((PlainSelect) select.getSelectBody()).getOffset().getOffsetJdbcParameter().toString());
    assertEquals(
        "?", ((PlainSelect) select.getSelectBody()).getFetch().getFetchJdbcParameter().toString());
    assertStatementCanBeDeparsedAs(select, statement);
  }
  ...
  public void testTop() throws JSQLParserException {
    ...
    assertEquals(
        3,
        select.getSelectBody(PlainSelect.class).getTop().getExpression(LongValue.class).getValue());
    assertStatementCanBeDeparsedAs(select, statement);
  ...
  public void testTopWithJdbcParameter() throws JSQLParserException {
    ...
    assertEquals(
        "name1",
        ((JdbcNamedParameter) ((PlainSelect) select.getSelectBody()).getTop().getExpression())
            .getName());
  ...
  public void testSelectItems() throws JSQLParserException {
    ...
    assertEquals(
        "mycol",
        ((Column) ((SelectExpressionItem) selectItems.get(1)).getExpression()).getColumnName());
    ...
    assertEquals(
        "schema.tab", ((AllTableColumns) selectItems.get(3)).getTable().getFullyQualifiedName());
    assertEquals(
        "mytab.mycol2",
    ...
    plainSelect = (PlainSelect) select.getSelectBody();
    assertEquals(
        "MYID", ((SelectExpressionItem) plainSelect.getSelectItems().get(0)).getAlias().getName());
    assertStatementCanBeDeparsedAs(select, statement);
  }
  ...
  public void testUnion() throws JSQLParserException {
    ...
    assertEquals(
        "mytable", ((Table) ((PlainSelect) setList.getSelects().get(0)).getFromItem()).getName());
    assertEquals(
        "mytable3", ((Table) ((PlainSelect) setList.getSelects().get(1)).getFromItem()).getName());
    assertEquals(
        "mytable2", ((Table) ((PlainSelect) setList.getSelects().get(2)).getFromItem()).getName());
    assertEquals(
        3,
  ...
  public void testJoin() throws JSQLParserException {
    ...
    assertEquals(
        "tab2", ((Table) plainSelect.getJoins().get(0).getRightItem()).getFullyQualifiedName());
    assertEquals(
        "tab1.id",
    ...
    assertEquals(
        "tab3", ((Table) plainSelect.getJoins().get(1).getRightItem()).getFullyQualifiedName());
    ...
    assertEquals(
        "id2", plainSelect.getJoins().get(0).getUsingColumns().get(1).getFullyQualifiedName());
    assertStatementCanBeDeparsedAs(select, statement);
    ...
    assertEquals(
        "bar", ((Table) plainSelect.getJoins().get(0).getRightItem()).getFullyQualifiedName());
  ...
  public void testFunctions() throws JSQLParserException {
    ...
    PlainSelect plainSelect = (PlainSelect) select.getSelectBody();
    assertEquals(
        "max", ((SelectExpressionItem) plainSelect.getSelectItems().get(0)).getAlias().getName());
    assertStatementCanBeDeparsedAs(select, statement);
    ...
    plainSelect = (PlainSelect) select.getSelectBody();
    assertEquals(
        "myavg", ((SelectExpressionItem) plainSelect.getSelectItems().get(1)).getAlias().getName());
    assertStatementCanBeDeparsedAs(select, statement);
    ...
    assertEquals(
        "b", ((Column) fun.getParameters().getExpressions().get(1)).getFullyQualifiedName());
    assertTrue(
  ...
  public void testGroupBy() throws JSQLParserException {
    ...
    assertEquals(
        "tab1.b",
        ((Column) plainSelect.getGroupBy().getGroupByExpressions().get(0)).getFullyQualifiedName());
    assertStatementCanBeDeparsedAs(select, statement);
    ...
    assertEquals(
        2, ((LongValue) plainSelect.getGroupBy().getGroupByExpressions().get(0)).getValue());
    assertEquals(
        3, ((LongValue) plainSelect.getGroupBy().getGroupByExpressions().get(1)).getValue());
    assertStatementCanBeDeparsedAs(select, statement);
  }
  ...
  public void testOrderBy() throws JSQLParserException {
    ...
    assertEquals(
        "b", ((Column) plainSelect.getOrderByElements().get(1).getExpression()).getColumnName());
    ...
    assertEquals(2, plainSelect.getOrderByElements().size());
    assertEquals(
        "a", ((Column) plainSelect.getOrderByElements().get(0).getExpression()).getColumnName());
    assertEquals(
        2, ((LongValue) plainSelect.getOrderByElements().get(1).getExpression()).getValue());
    assertStatementCanBeDeparsedAs(select, statement);
  }
  ...
  public void testNotLikeWithNotBeforeExpression() throws JSQLParserException {
    ...
    assertEquals(
        "test",
        ((StringValue) ((LikeExpression) notExpr.getExpression()).getRightExpression()).getValue());
  ...
  public void testWith() throws JSQLParserException {
    String statement =
        "WITH DINFO (DEPTNO, AVGSALARY, EMPCOUNT) AS "
  ...
  public void testOracleJoin2_1() throws JSQLParserException {
    ...
    for (String value : values) {
      assertSqlCanBeParsedAndDeparsed(
          "SELECT * FROM tabelle1, tabelle2 WHERE tabelle1.a" + value + " = tabelle2.b", true);
    }
  }
  ...
  //        String stmt = "select EXTRACT(DAY FROM (SYSDATE - to_date('20180101', 'YYYYMMDD' ) ) DAY
  // TO SECOND) from dual";
  //        assertSqlCanBeParsedAndDeparsed(stmt);
  //    }
  @Test
  ...
  public void testReservedKeyword() throws JSQLParserException {
    final String statement =
        "SELECT cast, do, extract, first, following, last, materialized, nulls, partition, range, row, rows, siblings, value, xml FROM tableName"; // all of these are legal in SQL server; 'row' and 'rows' are not legal on Oracle, though;
    final Select select = (Select) parserManager.parse(new StringReader(statement));
    assertStatementCanBeDeparsedAs(select, statement);
  }
  ...
  public void testJsonExpression() throws JSQLParserException {
    ...
    // assertSqlCanBeParsedAndDeparsed("SELECT * FROM sales WHERE CAST(sale->'items'->>'quantity' AS
    // integer)  = 2");
    assertSqlCanBeParsedAndDeparsed(
  ...
  //        assertSqlCanBeParsedAndDeparsed("WITH actor AS (SELECT 'b' aid FROM DUAL) SELECT aid
  // FROM actor UNION WITH actor2 AS (SELECT 'a' aid FROM DUAL) SELECT aid FROM actor2");
  //    }
  ...
  public void testOracleHint() throws JSQLParserException {
    ...
    assertOracleHintExists(
        "SELECT /*+ MORE HINTS POSSIBLE */ * FROM mytable", true, "MORE HINTS POSSIBLE");
    assertOracleHintExists(
        "SELECT /*+   MORE\nHINTS\t\nPOSSIBLE  */ * FROM mytable", true, "MORE\nHINTS\t\nPOSSIBLE");
    assertOracleHintExists(
        "SELECT /*+ leading(sn di md sh ot) cardinality(ot 1000) */ c, b FROM mytable",
        true,
        "leading(sn di md sh ot) cardinality(ot 1000)");
    assertOracleHintExists(
    ...
    assertOracleHintExists(
        ...
        "CLUSTER");
    assertOracleHintExists(
        "(SELECT * from t1) UNION (select /*+ CLUSTER */ * from dual)", true, null, "CLUSTER");
    assertOracleHintExists(
  ...
  public void testSpeedTestIssue235_2() throws IOException, JSQLParserException {
    String stmt =
        IOUtils.toString(
            SelectTest.class.getResourceAsStream("large-sql-issue-235.txt"),
            Charset.forName("UTF-8"));
  ...
  //        assertSqlCanBeParsedAndDeparsed("SELECT choice.response_choice_id AS uuid, choice.digit
  // AS digit, choice.text_response AS textResponse, choice.voice_prompt AS voicePrompt,
  ...
  // ref.confdeltype when 'c' then 0::int2 when 'n' then 2::int2 when 'd' then 4::int2 when 'r' then
  // 1::int2 else 3::int2 end as delete_rule, ref.conname as fk_name, cn.conname as pk_name, case
  // when ref.condeferrable then case when ref.condeferred then 5::int2 else 6::int2 end else
  // 7::int2 end as deferrablity from ((((((( (select cn.oid, conrelid, conkey, confrelid, confkey,
  // generate_series(array_lower(conkey, 1), array_upper(conkey, 1)) as i, confupdtype, confdeltype,
  // conname, condeferrable, condeferred from pg_catalog.pg_constraint cn, pg_catalog.pg_class c,
  // pg_catalog.pg_namespace n where contype = 'f' and conrelid = c.oid and relname = 'business' and
  // n.oid = c.relnamespace and n.nspname = 'public' ) ref inner join pg_catalog.pg_class c1 on
  // c1.oid = ref.conrelid) inner join pg_catalog.pg_namespace n1 on n1.oid = c1.relnamespace) inner
  // join pg_catalog.pg_attribute a1 on a1.attrelid = c1.oid and a1.attnum = conkey[i]) inner join
  // pg_catalog.pg_class c2 on c2.oid = ref.confrelid) inner join pg_catalog.pg_namespace n2 on
  // n2.oid = c2.relnamespace) inner join pg_catalog.pg_attribute a2 on a2.attrelid = c2.oid and
  // a2.attnum = confkey[i]) left outer join pg_catalog.pg_constraint cn on cn.conrelid =
  // ref.confrelid and cn.contype = 'p') order by ref.oid, ref.i", true);
  //        } catch (Exception ex) {
  //            ex.printStackTrace();
  //            throw ex;
  //        }
  ...
  //        assertSqlCanBeParsedAndDeparsed("select aa.* , t.* from accenter.all aa, (select a.*
  // from pacioli.emc_plan a) t");
  //    }
  //
  ...
  public void testIssue566LargeView() throws IOException, JSQLParserException {
    String stmt =
        IOUtils.toString(
            SelectTest.class.getResourceAsStream("large-sql-issue-566.txt"),
            Charset.forName("UTF-8"));
    assertSqlCanBeParsedAndDeparsed(stmt, true);
  }
  ...
  public void testAndOperator() throws JSQLParserException {
    ...
    assertStatementCanBeDeparsedAs(
        parsed, "SELECT name FROM customers WHERE name = 'John' && lastname = 'Doh'");
  }
  ...
  //        assertSqlCanBeParsedAndDeparsed("SELECT ( STUFF( (SELECT '|' + person_name FROM person
  // JOIN person_group ON person.person_id = person_group.person_id WHERE person_group.group_id = 1
  // FOR XML PATH(''), TYPE).value('.', 'varchar(max)'),1,1,'')) AS person_name");
  //    }
  @Test
  ...
  public void testIssue848_4() throws JSQLParserException {
    assertSqlCanBeParsedAndDeparsed(
        "select c1 from T1 where someFunc(select f1 from t2 where t2.id = T1.key) = 10", true);
  }
  ...
  public void testLimitClauseDroppedIssue845() throws JSQLParserException {
    assertEquals(
        ...
        CCJSqlParserUtil.parse("SELECT * FROM employee ORDER BY emp_id OFFSET 2 LIMIT 10")
            .toString());
  }
  ...
  public void testWrongParseTreeIssue89() throws JSQLParserException {
    Select unionQuery =
        (Select)
            CCJSqlParserUtil.parse("SELECT * FROM table1 UNION SELECT * FROM table2 ORDER BY col");
    SetOperationList unionQueries = (SetOperationList) unionQuery.getSelectBody();

    assertThat(unionQueries.getSelects())
        .extracting(select -> (PlainSelect) select)
