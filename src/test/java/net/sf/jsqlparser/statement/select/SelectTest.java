...
public class SelectTest {
    ...
    public void testTopWithJdbcParameter() throws JSQLParserException {
        ...
        assertEquals(1, (int) ((JdbcParameter) ((PlainSelect) select.getSelectBody()).getTop().getExpression()).getIndex());
        assertStatementCanBeDeparsedAs(select, statement);

        statement = "select top :name1 foo from bar";
        select = (Select) parserManager.parse(new StringReader(statement));
        assertEquals("name1", ((JdbcNamedParameter) ((PlainSelect) select.getSelectBody()).getTop().getExpression()).getName());
    ...
    public void testDouble() throws JSQLParserException {
        ...
        assertEquals(1e2,
            ((DoubleValue) ((BinaryExpression) ((PlainSelect) select.getSelectBody()).getWhere()).getRightExpression()).getValue(), 0);
        assertStatementCanBeDeparsedAs(select, statement);
    ...
    @Test
    public void testTeradataSelect01() throws JSQLParserException {
        assertSqlCanBeParsedAndDeparsed("SELECT DATE, TIME");
    }

    @Test
    public void testTeradataSelect02() throws JSQLParserException {
        assertSqlCanBeParsedAndDeparsed("SELECT x1, x2 FROM t1, t2 WHERE t1.x1 = t2.x2");
    }

    @Test
    public void testTeradataSelect03() throws JSQLParserException {
        assertSqlCanBeParsed("SELECT x1 FROM t1 WHERE x1 IN (SELECT x2 FROM t2)");
    }

    @Test
    public void testTeradataUsing1() throws JSQLParserException {
        parserManager.parse(new StringReader("USING (a INTEGER) SELECT TOP :a * FROM employee;"));
        parserManager.parse(new StringReader("USING SYS_DAT(VARBYTE(255)) INS personnel.inslogtable(LOGTYPE,SYSINFO) VALUES(0,:SYS_DAT)"));
        parserManager.parse(new StringReader("USING SYS_DAT(VARBYTE(255)), b(Integer) INS personnel.inslogtable(LOGTYPE,SYSINFO, b) VALUES(0,:SYS_DAT, :b)"));
        parserManager.parse(new StringReader("USING (a INTEGER, b INTEGER, c INTEGER) SELECT TOP :a *  FROM t1;"));
        parserManager.parse(new StringReader("USING (a CHARACTER(10), b CHARACTER(10)) SELECT test.value_expression(:a, :b) AS cve  FROM t1 WHERE t1.a1 = :a  AND   t1.b1 = :b;"));
    }
}
